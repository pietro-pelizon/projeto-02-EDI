...
static void comando_a(char *buffer, lista *formas, lista *anteparos, FILE *arquivo_txt) {
    ...
    while (head != NULL) {
        ...
        if (id_forma >= id_i && id_forma <= id_j) {
            ...
            const char* tipo_nome[] = {"CIRCULO", "RETANGULO", "LINHA", "TEXTO", "ANTEPARO"};
...
destruídas
SVG: desenhar a região de visibilidade da
bomba. A região de visibilidade deve ser
desenhada em um novo arquivo cujo nome
tem o sufixo sfx ou no próprio arquivo svg
final, caso sfx seja “-”.
*/

void comando_d(int threshold_i, char tipo_ord, char *buffer, lista *formas, lista *anteparos, FILE *arquivo_txt,
    const char *path_base_svg, const char *path_saida) {
    ...
    poligono *visibilidade = calc_regiao_visibilidade(bomba, anteparos, tipo_ord, raio_max, threshold_i);


    fprintf(arquivo_txt, "[*] d %lf %lf %s\n", x_impacto, y_impacto, sfx);
    ...
    while (head != NULL) {
        ...
        if (forma_sobrepoe_poligono(f, visibilidade)) {
    ...
    if (strcmp(sfx, "-") != 0) {
        ...
        if (svg_vis) {
            insere_poligono_visibilidade(svg_vis, visibilidade, "#FF0000");
            fecha_svg(svg_vis);
        }
    }
...
visibilidade tem suas cores de
...
static void comando_p(lista *formas, lista *anteparos, FILE *arquivo_txt, char *buffer,
    ...
    double x_impacto, y_impacto; char sfx[64] = "-"; char cor[16] = "";
...
static void comando_cln(lista *formas, lista *anteparos, FILE *arquivo_txt,
    int threshold_i, char tipo_ord, const char *path_saida, const char *path_base_svg, char *buffer) {
    ...
    while (head != NULL) {
        ...
        if (forma_sobrepoe_poligono(f, visibilidade)) {
            ...
            const char* tipo_nome[] = {"CIRCULO", "RETANGULO", "LINHA", "TEXTO", "ANTEPARO"};
            fprintf(arquivo_txt, "Forma original - ID -> %i\n | Tipo -> %s\n ", get_id_forma(f), tipo_nome[get_tipo_forma(f)]);
